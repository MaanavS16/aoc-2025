...
struct union_find {
  ...
  std::size_t find(coord_id_t i) const { return parent_[i] == i ? i : find(parent_[i]); }
...
uint64_t solve(std::vector<coord_t> const& input) {
  ...
  std::pair<coord_id_t, coord_id_t> last_connect;
  while (!dist_index.empty()) {
    auto const [curr_dist, ids] = dist_index.top();
    auto const [pid1, pid2] = ids;
    dist_index.pop();
    if (uf.find(pid1) != uf.find(pid2)) {
      uf.merge(pid1, pid2);
      last_connect = {pid1, pid2};
    }
  }
  return input[last_connect.first][0] * input[last_connect.second][0];
}
